...
struct CreateBookReview: Migration {
    ...
    func prepare(on database: Database) -> EventLoopFuture<Void> {
        ...
        return database.schema(BookReview.schema)
            ...
            .field(bookReview.$characterRating.key, .int, .sql(defaultRating))
            .field(bookReview.$infoRating.key, .int, .sql(defaultRating))
            .field(bookReview.$targetRating.key, .int, .sql(defaultRating))
            .field(bookReview.$writeRating.key, .int, .sql(defaultRating))
